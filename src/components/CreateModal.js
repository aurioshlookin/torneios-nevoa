...
const CreateModal = ({ onClose, onCreate }) => {
    ...
    const handleChange = (e) => {
        ...
        if(['matchFormat', 'structure', 'shuffle'].includes(key)) {
            const desc = `
                ${FORMAT_INFO.matches[newForm.matchFormat] || ''} • 
                ${FORMAT_INFO.structure[newForm.structure] || ''} • 
                ${FORMAT_INFO.shuffle[newForm.shuffle] || ''}
            `;
            setDescPreview(desc);
        }
    };
        ...
        <div className="fixed inset-0 z-[60] bg-black/80 flex items-center justify-center backdrop-blur-sm p-4 animate-fade-in">
            <div className="bg-slate-800 w-full max-w-2xl rounded-xl border border-slate-600 shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
                    ...
                    <form id="create-form" onSubmit={handleSubmit} className="space-y-4">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="text" id="t-title" required placeholder="Ex: Exame Chunin" className="w-full bg-slate-900 border border-slate-700 rounded-lg p-3 focus:border-green-500 outline-none text-white" onChange={handleChange} />
                            </div>
                            <div>
                                <label className="block text-xs uppercase tracking-wider text-slate-500 mb-1 font-bold ml-1">Data/Horário</label>
                                <input type="text" id="t-date" required placeholder="Ex: Sábado às 19:00" className="w-full bg-slate-900 border border-slate-700 rounded-lg p-3 focus:border-green-500 outline-none text-white" onChange={handleChange} />
                            </div>
                        </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-3 bg-slate-900/50 p-4 rounded-lg border border-slate-700/50">
                            <div>
                                ...
                                <select id="t-matchFormat" className="w-full bg-slate-800 border border-slate-600 rounded p-2 text-sm focus:border-green-500 outline-none text-white" onChange={handleChange} value={form.matchFormat}>
                            ...
                            <div>
                                ...
                                <select id="t-structure" className="w-full bg-slate-800 border border-slate-600 rounded p-2 text-sm focus:border-green-500 outline-none text-white" onChange={handleChange} value={form.structure}>
                            ...
                            <div>
                                ...
                                <select id="t-shuffle" className="w-full bg-slate-800 border border-slate-600 rounded p-2 text-sm focus:border-green-500 outline-none text-white" onChange={handleChange} value={form.shuffle}>
                            ...
                            <div className="col-span-full mt-2">
                                <p className="text-xs text-yellow-500/80 italic border-l-2 border-yellow-500 pl-2">{descPreview}</p>
                            </div>
                        </div>

                        <div>
                            <label className="block text-xs uppercase tracking-wider text-slate-500 mb-1 font-bold ml-1">Descrição</label>
                            <textarea id="t-description" rows="4" required placeholder="Regras..." className="w-full bg-slate-900 border border-slate-700 rounded-lg p-3 focus:border-green-500 outline-none text-white" onChange={handleChange}></textarea>
                        </div>
                    </form>
                </div>
                ...
                <div className="p-4 border-t border-slate-700 bg-slate-900/50 flex justify-end gap-3">
                    ...
                    <button onClick={() => document.getElementById('create-form').requestSubmit()} className="bg-green-600 hover:bg-green-500 text-white font-bold py-2 px-6 rounded-lg shadow-lg flex items-center gap-2">
